...
boolean P_CheckMissileRange(mobj_t * actor)
{
    ...
    const int dist_cap = (gameskill == sk_ultranm) ? 140 : 200;
    if(dist > dist_cap)
    {
        dist = dist_cap;
    }
    ...
    && actor->type != MT_MINOTAUR
    && actor->type != MT_SORCERER2
    && dist < 85)
    {
        dist = 85;
    }
...
boolean P_TryWalk(mobj_t * actor)
{
    ...
    if(gameskill == sk_ultranm)
    {
        ...
        actor->movecount = rand & (actor->type == MT_WIZARD || actor->type == MT_SNAKE ? 3 : 1);
        if(rand > 234)
        {
            actor->movecount += 4;
        }
...
void A_MinotaurDecide(mobj_t *actor, player_t *player, pspdef_t *psp)
{
    ...
    else if((is_firewall_attack_distance && P_Random() < (gameskill == sk_ultranm ? 160 : 220))
...
void P_DropItem(mobj_t * source, mobjtype_t type, int special, int chance)
{
    ...
    mo->flags |= MF_DROPPED;
    mo->flags &= ~MF_COUNTITEM; // [JN] Don't count dropped items
    mo->health = special;
}
...
void A_ESound(mobj_t *mo, player_t *player, pspdef_t *psp)
{
    ...
    // using standard S_StartSound, use S_StartSoundAmbient which is calculating
    // both values correctly and making sounds always playable.

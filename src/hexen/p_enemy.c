...
{
    P_SeekerMissile(actor, ANG1 * 2, ANG1 * (gameskill == sk_ultranm ? 6 : 3));
}
...
{
    if(P_Random() < ((gameskill == sk_ultranm) ? 100 : 220))
    {
        return;
    }
    else
...
static void DragonSeek(mobj_t * actor, angle_t thresh, angle_t turnMax)
{
    ...
    {                           // attack the destination mobj if it's attackable
        ...
        if (actor->angle - R_PointToAngle2(actor->x, actor->y,
                                               target->x,
                                               target->y) < ANG45 / 2)
        {
        ...
        if (actor->target && P_Random() < 200)
        {
            ...
            for (i = 0; i < 5; i++)
            {
                ...
                angleToSpot = R_PointToAngle2(actor->x, actor->y,
                                              mo->x, mo->y);
                if (angleToSpot - angleToTarget < bestAngle)
                {
                    bestAngle = angleToSpot - angleToTarget;
                    bestArg = i;
                }
            }
            if (bestArg != -1)
    ...
    switch (mode)
    {
        ...
        case SORC_STOPPING:    // Balls stopping
            if ((parent->special2.i == actor->type) &&
                ...
                (angle - (parent->angle >> ANGLETOFINESHIFT) <
                 (30 << 5)))
            {
                // Can stop now
